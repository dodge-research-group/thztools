...
def calcnoise(t, x):
    ...
    v0 = np.mean(np.var(x, 1)) * np.array([1, np.sqrt(np.finfo(float).eps),
                                           np.sqrt(np.finfo(float).eps)])

    mu0 = np.mean(x, axis=1)
    param['delay'] = {'v0': v0, 'mu0': mu0, 'ts': ts}
    eta0 = tdnoisefit(x, param['delay'], fix['delay'],
                      ignore['delay'])[0]['eta']
    ...
    xadjusted = airscancorrect(x, {'a': all_fit[0]['a'],
                                   'eta': all_fit[0]['eta'],
                                   'ts': ts})

    # data ouput throuhg calc dictionary
    calc = {'p': output['p'], 'nllmin': output['nllmin'], 'vest': vest,
            'verr': verr, 'muest': muest, 'muerr': muerr, 'aest': aest,
            'aerr': aerr, 'etaest': etaest, 'etaerr': etaerr, 't': t, 'x': x,
            'zeta': zeta, 'delta': delta, 'vtot': vtot, 'vtothigh': vtothigh,
            'vtotlow': vtotlow, 'xadjusted': xadjusted,
            'sigma_alpha': sigmaalphastar, 'sigma_beta': sigmabetastar,
            'sigma_tau': sigmataustar}
    print('=====================================================')
    print('sigma estimates:', np.round(sigmaalphastar, 4), "pA, ",
