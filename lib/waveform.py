    ...
    def __init__(self, filename):
        ...
        for line in thz_file:
            ...
            if attributes[0].isalpha():
                self.local_attributes[attributes[0]] = attributes[1].rstrip("\n")
                ...
                self.time, self.amplitude = np.genfromtxt('Scan.thz', comments='#', unpack=True, dtype=None,
                                                          skip_header=3, encoding=None)

                                                          
                                                          
class Thzwaveform2:
    def _init__(self,filename, local_attributes, Acquisition_time, Wait_time, Time_constant, time, amplitude):
        self.filename = filename
	...
	thz_file = open(filename, 'r')
        for line in thz_file:
            ...
            if attributes[0].isalpha():
                ...
                time, amplitude = np.genfromtxt('Scan.thz', comments='#', unpack=True, dtype=None,
                                                          skip_header=3, encoding=None)
