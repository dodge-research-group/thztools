...
def fit_main(
    ...
    *,
    noise_parms: ArrayLike | None = [1, 1, 1],
    p_bounds: ArrayLike | None = [np.NINF, np.inf],
    f_args: ArrayLike | None = None,
    jac: Callable | None = None,
) -> tuple[dict]:
    r"""
    Fit THz time-domain data to a transfer function.

    Computes the noise on the input ``xx`` and output ``yy`` time series using "noiseamp". 
    Then uses the total residuals generated by "costfuntls" to fit the input and output to
    the transfer function.

    Parameters
    ----------
        fun : callable
            Transfer function, in the form fun(theta,w,*f_args), +iwt convention.
        p0 : array_like
        ...
        ts : float
            Sampling time.
        nois_parms : None or array_like, optional
    ...
    def function(_theta, _w):
        if f_args == None:
            return fun(_theta, _w)
        else:
            return fun(_theta, _w, f_args)

    def td_fun(_p, _x):
        _y = irfft(rfft(_x) * function(_p, w), n=n)
        return _y
