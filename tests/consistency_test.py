        ...
        for i in range(0, nsigma.shape[0]):
            for j in range(0, nmu.shape[0]):
                for k in range(0, nt.shape[0]):
                    sigma = np.array(
                        file[file_set['noisevar']['sigma'][i, j, k]])[0]
                    mu = np.array(
                        file[file_set['noisevar']['mu'][i, j, k]])[0]
                    t = np.array(
                        file[file_set['noisevar']['T'][i, j, k]])[0, 0]
                    vmu = np.array(
                        file[file_set['noisevar']['Vmu'][i, j, k]])[0]
        ...
        for i in range(0, dfn.shape[0]):
            for j in range(0, dft.shape[0]):
                for k in range(0, dft0.shape[0]):
                    n = np.array(
                        file[file_set['thzgen']['N'][i, j, k]])[0, 0]
                    t = np.array(
                        file[file_set['thzgen']['T'][i, j, k]])[0, 0]
                    t0 = np.array(
                        file[file_set['thzgen']['t0'][i, j, k]])[0, 0]
                    y = np.array(
                        file[file_set['thzgen']['y'][i, j, k]])[0]
                    ...
                    for m in range(0, dfsigmax.shape[0]):
                        for n in range(0, dfsigmay.shape[0]):
                            for p in range(0, dfwfft.shape[0]):
                                ...
                                sigmax = np.array(
                                    file[file_set['costfunlsq'][
                                        'sigmax'][i, j, k, m, n, p]])[0]
                                sigmay = np.array(
                                    file[file_set['costfunlsq'][
                                        'sigmay'][i, j, k, m, n, p]])[0]
                                # wfft = np.array(
                                # file[Set['costfunlsq'][
                                # 'wfft'][i, j, k, l, m, n]])[0]
                                res = np.array(
                                    file[file_set['costfunlsq'][
                                        'res'][i, j, k, m, n, p]])[0]
                                fpy = costfunlsq(fun, theta, xx, yy,
                                                 sigmax, sigmay, i + 1)
        ...
        for i in range(0, ntheta.shape[0]):
            for j in range(0, nn.shape[0]):
                for k in range(0, nts.shape[0]):
                    theta = np.array(
                        file[file_set['tdtf']['theta'][i, j, k]])[0]
                    n = np.array(
                        file[file_set['tdtf']['N'][i, j, k]])[0, 0]
                    ts = np.array(
                        file[file_set['tdtf']['ts'][i, j, k]])[0, 0]
                    h = np.array(
                        file[file_set['tdtf']['h'][i, j, k]])
        ...
        for i in range(0, dfx.shape[1]):
            for j in range(0, df_param.shape[0]):
                for k in range(0, dfvarargin.shape[0]):
                    ...
                    param = {'logv': np.array(
                        file[p_struct[0, j, 0]]['logv'])[0],
                             'mu': np.array(
                                 file[p_struct[0, j, 0]]['mu'])[0],
                             'a': np.array(
                                 file[p_struct[0, j, 0]]['A'])[0],
                             'eta': np.array(
                                 file[p_struct[0, j, 0]]['eta'])[0],
                             'ts': np.array(
                                 file[p_struct[0, j, 0]]['ts'])[0],
                             'D': np.array(
                                 file[p_struct[0, j, 0]]['D'])}
                    varargin = {'logv': bool(np.array(
                        file[varargin_struct[k, 0, 0]]['logv'])),
                                'mu': bool(np.array(
                                    file[varargin_struct[k, 0, 0]]['mu'])),
                                'a': bool(np.array(
                                    file[varargin_struct[k, 0, 0]]['A'])),
                                'eta': bool(np.array(
                                    file[varargin_struct[k, 0, 0]]['eta']))}
                    nll = np.array(
                        file[file_set['tdnll']['nll'][k, j, i]])[0, 0]
                    # gradnll = np.array(
    ...
    with h5py.File(fname, 'r') as file:
        ...
        for i in range(0, 1):
            for j in range(0, 1):
                ...
                p = {'var': np.array(file[p_matlab[j, 0]]['var'])[0],
