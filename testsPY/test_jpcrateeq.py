...
def test():
    with h5py.File('jpcrateeq_test_data.mat', 'r') as file:
        set = file['Set']
        ...
        for i in range(0, dft.shape[0]):
            for j in range(0, dftheta.shape[0]):
                ...
                np.testing.assert_allclose(dndt, resultPy[0])
                np.testing.assert_allclose(dvdt, resultPy[1])
                np.testing.assert_allclose(dpdt, resultPy[2])